			...
			return this.run(e, function(e) {
				...
				if (isIE) {
					function set() {
						...
						try {
							//Start - Fixed remove space issue (both word wrong)
							flag=false;
							if(/^\s+/.test(h)){
								flag=true;
							}
							//End - Fixed remove space issue (both word wrong)
		...
		getBookmark : function(type, normalized) {
			...
			if (rng.duplicate || rng.item) {
				...
				if (!rng.item) {
					...
					// Insert start marker
					rng.collapse();
					/* creating prob while saving with spellchecking - Start*/
					try{
						rng.pasteHTML('<span _mce_type="bookmark" id="' + id + '_start" style="' + styles + '">' + chr + '</span>');
					}
					catch (ex) {
						// IE throws unspecified error here while submit form
					}
					/*creating prob while saving with spellchecking - End */
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.SplitButton:tinymce.ui.MenuButton', {
		...
		renderHTML : function() {
			...
			/* by sanjay below condition */
			if(s.title!= 'Toggle spellchecker')
