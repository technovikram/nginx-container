...
(function($) {
    $.fn.extend({
        ...
        jtimepicker: function(options) {
            ...
            var defaults = {
                ...
                secLabel: 'sec',
		// set AM/PM combo
		meridiemCombo:'meridiemcombo',
		meridiemDefaultValue:'AM'
            };
		...
		//Added to create AM/PM combo in case of 12 hr mode.
		if(o.hourMode==12){
		 html += $this.createMeridiemCombo(o.meridiemCombo, o.meridiemDefaultValue);	
		}
    ...
    //Added to create combo for AM/PM in case of 12 hour mode.
    $.fn.createMeridiemCombo = function(id,defValue) {
        var html = '<select class="' + id + ' combo" name="' + id + '">';
        for(i = 0; i < 2; i ++) {
            var txt = i < 1 ? 'AM' : 'PM';
	    var key = i < 1 ? 'AM' : 'PM';
	    var selected = key == defValue ? ' selected="selected"' : '';
