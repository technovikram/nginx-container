...
$.widget(widgetName, {
	...
	*/
	...
	_populateFromMaster: function() {
		...
		//CWSi changes to hide first blank value.
		index = 0;		

		//alert(this.selectbox.get(0).options[0].value);
		
		var newLoopCountdown = loopCountdown;
		if(this.selectbox.get(0).options[0] != null && this.selectbox.get(0).options[0].value == ''){
			//index = 1;
			//newLoopCountdown = loopCountdown-1;
		}
		while(newLoopCountdown --) {
			thisOpt = options[index++];
			listBuilder.push('<li name="');
			listBuilder.push(thisOpt.index);
			listBuilder.push('"');
			//listBuilder.push(' class=');
			//listBuilder.push(newLoopCountdown %2 == 0 ? "ac_even" : "ac_odd");
			listBuilder.push('>');
	...
	setDimensions: function() {
		...
		/* CWSi Changes to view combobox for visible fields.
		...
		//CWSi change to view visible fields.	
		var testWidthInput = newSelectWidth - inputWidth;
		if(testWidthInput <22)newSelectWidth =inputWidth +21;
		
		//alert(inputWidth + '---' +newSelectWidth);	
...
$.extend($.ui.ufd, {
	...
	defaults: { // 1.7 default options location, see below
		...
		minWidth: 100, // don't autosize smaller then this. CWSi : Changed 50 to 100.
